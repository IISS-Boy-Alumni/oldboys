...
def create_app(config_name="development"):
    ...
    click.echo(f"Debug: Registered blueprints before load_blueprints: {list(app.blueprints.keys())}")
...
def load_plugins(app, global_template_variables, global_configs, config_name):
    click.echo(f"Debug: installed_packages in load_plugins: {installed_packages}")
...
def setup_flask_admin(app):
    ...
    admin.add_view(DefaultModelView(Alumni, db.session))
    admin.add_view(DefaultModelView(Teacher, db.session))
    admin.add_view(DefaultModelView(Tribute, db.session))
